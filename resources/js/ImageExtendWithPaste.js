// Copied from node_modules/quill-image-extend-module/index.js and uncommented this.uploadImg(), this.toBase64()
...
export class ImageExtend {
     ...
     */
    insertImg() {
        QuillWatch.active.quill.insertEmbed(QuillWatch.active.cursorIndex, 'image', QuillWatch.active.imgURL)
        QuillWatch.active.quill.update()
     ...
     */
    progress(pro) {
        QuillWatch.active.quill.root.innerHTML = QuillWatch.active.quill.root.innerHTML
            .replace(/\[uploading.*?\]/, '[' + 'uploading' + pro + ']')
    }
    ...
    uploadError() {
        QuillWatch.active.quill.root.innerHTML = QuillWatch.active.quill.root.innerHTML
            .replace(/<\s*img src="data:image.*?"[^>]*>/, '')
            .replace(/\[uploading.*?\]/, '[upload error]')
    }
